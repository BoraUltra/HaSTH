...
public class BuiltinResourcePackTestMod implements ModInitializer {
	public static final String MODID = "boraultra";
	...
	public void onInitialize() {
		...
		FabricLoader.getInstance().getModContainer(MODID)
				.map(container -> ResourceManagerHelper.registerBuiltinResourcePack(new Identifier(MODID, "hasth_pack"),
						container, ResourcePackActivationType.NORMAL))
				.filter(success -> !success)
				.ifPresent(success -> LOGGER.warn("Could not register built-in resource pack."));
		System.out.println("Sucess Initialize BuildinResourcePack");
	}
}